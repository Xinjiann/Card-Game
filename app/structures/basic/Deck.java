...
public class Deck {
    ...
    public void humanDeck() {
        ...
        for (int i = 0; i < 8; i++) {
            card = BasicObjectBuilders.loadCard(deck1Cards[i], deck1Units[i], i + 2, Card.class); // 3 initial cards so
            // index start from
            // 2
    ...
    public void aiDeck() {
        ...
        for (int i = 0; i < 8; i++) {
            card = BasicObjectBuilders.loadCard(deck2Cards[i], deck2Units[i], i + 2 + 2 * deck2Cards.length,
                    Card.class); // 3 initial cards so index start from 2
            this.cardList.add(card);
            card = BasicObjectBuilders.loadCard(deck2Cards[i], deck2Units[i],
                    i + 2 + 2 * deck2Cards.length + deck2Units.length, Card.class);
            this.cardList.add(card);
        }
