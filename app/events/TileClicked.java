...
import structures.basic.abilities.Provoke;
...
public class TileClicked implements EventProcessor {
	...
	public void playSpell(GameState gameState, ActorRef out, Tile clickedTile, Card selectedCard) {
		...
		if (monster.getHealth() == 0) {
			...
			if (selectedCard.getAbilityList() != null) {
				for (Ability ability : selectedCard.getAbilityList()) {
					if (ability.getWhenTOCall() == WhenToCall.death) {
						ability.execute(monster, gameState);
					}
	...
	public void summonUnit(GameState gameState, ActorRef out, Tile clickedTile, Card selectedCard) {
		...
		Monster monster = BasicObjectBuilders.loadMonsterUnit(selectedCard.getUnitConfigFiles(), selectedCard,
				gameState.getTurnOwner(), Monster.class);
		...
		if (selectedCard.getAbilityList() != null) {
			for (Ability ability : selectedCard.getAbilityList()) {
				...
				if (ability.getWhenTOCall() == WhenToCall.aura) {
					ability.execute(monster, gameState);
				}
				if (ability.getEffectAnimation() != null) {
					BasicCommands.playEffectAnimation(out, ability.getEffectAnimation(), clickedTile);
	...
	public void moveClick(Monster previousMonster, GameState gameState, ActorRef out, Tile clickedTile) {
		...
		movableTiles = gameState.getGameBoard().getMovableTiles(previous_x, previous_y, previousMonster.getMovesLeft());
		attachableTiles = gameState.getGameBoard().getAttachableTiles(previous_x, previous_y,
				previousMonster.getMovesLeft(), previousMonster.getAttackDistance());
		movableTiles.addAll(attachableTiles);
		...
		if ((!movableTiles.isEmpty())) {
			...
			if ((deltaX + deltaY) > previousMonster.getMovesLeft()) {
				...
				// remove provoke
				if (previousMonster.getAbilities() != null) {
					for (Ability theAbility : previousMonster.getAbilities()) {
						if (theAbility instanceof Provoke) {
							HashSet<Monster> monsterOnTiles = CommonUtils.getMonsterAround(previousMonster, gameState);
				...
				if (previousMonster.getAbilities() != null) {
					for (Ability theAbility : previousMonster.getAbilities()) {
						if (theAbility.getWhenTOCall() == WhenToCall.aura) {
							theAbility.execute(previousMonster, gameState);
						}
					}
				}
				// front end work
	...
	private void enemyClick(Monster clickedMonster, GameState gameState, ActorRef out, Tile clickedTile) {
		...
		if (Math.abs(previousTile.getTilex() - clickedTile.getTilex()) <= previousMonster.getAttackDistance()
				&& Math.abs(previousTile.getTiley() - clickedTile.getTiley()) <= previousMonster.getAttackDistance()) {
	...
	private void attack(Monster attacker, Monster defender, GameState gameState, ActorRef out, Tile previousTile,
			Tile clickedTile) {
		...
		if (!survived) {
			...
			if (defender.getAbilities() != null) {
				for (Ability ability : defender.getAbilities()) {
					if (ability.getWhenTOCall() == WhenToCall.death) {
						ability.execute(defender, gameState);
					}
					if (ability.getWhenTOCall() == WhenToCall.aura) {
						ability.execute(defender, gameState);
					}
	...
	public void moveAndAttack(Monster previousMonster, Monster clickedMonster, GameState gameState, ActorRef out,
		...
		Tile tileToGo = null;
		if (previousMonster.isBeenProvoke() && (!previousMonster.getProvokeOwners().contains(clickedMonster))) {
			HashSet<Monster> monsterOnTiles = CommonUtils.getMonsterAround(previousMonster, gameState);
			if (monsterOnTiles != null) {
				for (Monster monsterOnTile : monsterOnTiles)
					for (Ability a : monsterOnTile.getAbilities()) {
						if (a instanceof Provoke) {
							clickedMonster = monsterOnTile;
							attack(previousMonster, clickedMonster, gameState, out,
									gameState.gameBoard.getTile(previousMonster.getPosition().getTilex(),
											previousMonster.getPosition().getTiley()),
									gameState.gameBoard.getTile(clickedMonster.getPosition().getTilex(),
											clickedMonster.getPosition().getTiley()));
						}
						break;
					}
			}
