...
public class AiPlayGame {
	...
	public void paly() {
		...
		for (Monster monster : allAvailableUnit) {
			if (!allMovableOnlyUnit.contains(monster)) {
				if (!monster.isFrozen()) {
					...
					if (Math.abs(monster.getPosition().getTilex() - bestTarget.getTilex()) < 2
							&& Math.abs(monster.getPosition().getTiley() - bestTarget.getTiley()) < 2) {
						// attack
						tileClicked.attack(
								monster, bestTarget.getUnitOnTile(), gameState, out, gameState.getGameBoard()
										.getTile(monster.getPosition().getTilex(), monster.getPosition().getTiley()),
								bestTarget);
					} else {
						// move and attack
						tileClicked.moveAndAttack(
								monster, bestTarget.getUnitOnTile(), gameState, out, gameState.getGameBoard()
										.getTile(monster.getPosition().getTilex(), monster.getPosition().getTiley()),
								bestTarget);
					}
		...
		for (Monster monster : allMovableOnlyUnit) {
			...
			int delta = Math.abs(bestTile.getTilex() - monster.getPosition().getTilex())
					+ Math.abs(bestTile.getTiley() - monster.getPosition().getTiley());
