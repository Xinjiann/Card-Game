...
public class AiPlayGame {
    ...
    public void paly() {
        ...
        for (Monster monster : allAvailableUnit) {
            if (!allMovableOnlyUnit.contains(monster)) {
                if (!monster.isFrozen()) {
                    ...
                    ArrayList<Tile> tileList = gameState.gameBoard.getAttachableTiles(x, y, movesLeft, gameState, attackDistance);
                    ...
                    if (Math.abs(monster.getPosition().getTilex() - bestTarget.getTilex()) < 2 && Math.abs(monster.getPosition().getTiley() - bestTarget.getTiley()) < 2) {
                        // attack
                        tileClicked.attack(monster, bestTarget.getUnitOnTile(), gameState, out, gameState.getGameBoard().getTile(monster.getPosition().getTilex(), monster.getPosition().getTiley()), bestTarget);
                    } else {
                        // move and attack
                        tileClicked.moveAndAttack(monster, bestTarget.getUnitOnTile(), gameState, out, gameState.getGameBoard().getTile(monster.getPosition().getTilex(), monster.getPosition().getTiley()), bestTarget);
                    }
                }
            }
        }
        ...
        for (Monster monster : allMovableOnlyUnit) {
            ...
            int x = monster.getPosition().getTilex();
            int y = monster.getPosition().getTiley();
            ArrayList<Tile> movableTiles = gameState.getGameBoard().getMovableTiles(x, y, monster.getMovesLeft(), gameState);
            Tile bestTile = getBestMoveTarget(movableTiles);
            int delta = Math.abs(bestTile.getTilex() - monster.getPosition().getTilex()) + Math.abs(bestTile.getTiley() - monster.getPosition().getTiley());
    ...
    private ArrayList<Monster> getAllMovableOnlyUnit(ArrayList<Monster> allAvailableUnit) {
        ArrayList<Monster> list = new ArrayList<>();
        for (Monster m : allAvailableUnit) {
            ArrayList<Tile> attachableTiles = gameState.gameBoard.getAttachableTiles(m.getPosition().getTilex(), m.getPosition().getTiley(), m.getMovesLeft(), gameState, m.getAttackDistance());
    ...
    private ArrayList<Tile> rankAttachableTargetUnit(HashMap<Monster, ArrayList<Tile>> attachableTiles) {
    ...
    private ArrayList<Monster> rankAttachableUnit(HashMap<Monster, ArrayList<Tile>> allMovableUnit) {
        ...
        List<Entry<Monster, Integer>> list = new ArrayList<Entry<Monster, Integer>>(scoreMap.entrySet());
